...
**How to use:**
- Add a new entry at the top (newest first). Copy the template block below.
...
### `/release` skill for version tagging and GitHub Releases
...
### Dedicated `prompts/` folder for issue session briefs
...
### `/prepare-issue` skill for auto-filling issue prompts
...
### Auto-generated changelog / release notes skill
...
**Sketch of the idea:** Ship a small script (e.g. `bin/install-workflow-kit`) that runs the copy steps, scaffolds `Design/adr/` and `notes/`, renders `CLAUDE.md` from the template, and makes the initial commit. Keep the manual flow documented as a fallback so nothing is hidden.
...
### Write `docs/claude-code-guide.md`
...
### Example / starter projects gallery
...
### Optional `--with-docs` flag on install
...
- Deferred: a refresh/update command to re-sync copied docs with the kit source.
...
**Options in mind:**
- Single flat notepad file (this option) — lowest friction, easy to scan.
- One file per idea under `notes/ideas/` — better for long-lived or detailed ideas, more overhead.
- External tracker (Linear, GitHub issues) — heavier, loses the "in-repo, editable alongside code" benefit.

**Open questions:**
- Should the `adr-writer` skill learn to read entries directly from this file (e.g. by title), or is copy-paste fine?
- Do we want a `/capture-idea` slash command that appends a new block with today's date pre-filled?
- Should entries auto-migrate to an archive section once their ADR ships?

**Consequences to think through:**
- Easier: low-friction capture; fewer half-baked ADRs; clear pipeline from idea → ADR.
- Harder: one more file to keep tidy; risk of the notepad becoming a graveyard if entries aren't groomed.
- Maintenance: needs a light grooming pass occasionally to prune stale `idea` entries or promote ripe ones.
- Deferred: richer tooling (slash command, auto-archive, skill integration) — revisit once the plain notepad proves useful.
