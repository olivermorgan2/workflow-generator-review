...
**MAJOR** per [ADR-026](design/adr/adr-026-kit-versioning-policy.md): renames the load-bearing root directory `Design/` to `design/` for casing consistency with the kit's other lowercase root directories. Breaking change for every existing target-project install.

### Breaking changes

- **Renamed `Design/` → `design/`** kit-wide ([ADR-045](design/adr/adr-045-rename-design-directory-lowercase.md), [#80](https://github.com/olivermorgan2/workflow-generator/issues/80)). Pure case fix — preserves the directory's current breadth (ADRs + `state.md` + `ai-summary.md` + optional MVP/decisions/planning docs). Touches ~1,186 path-string occurrences across 179 files: all 19 `skills/*/SKILL.md`, `templates/`, `docs/`, `examples/`, `archive/`, `notes/`, `prompts/`, `bin/` scripts, root files (`CLAUDE.md`, `README.md`), `.github/` configs, and the bodies of 16 accepted ADRs (the latter authorised by ADR-044's mechanical-rewrite exception). ADR-005's substantive decision (generate core docs into the target repo) stands; only the casing changes. ADR-005 status: superseded by ADR-045 on the casing question only.

- **Migration for prior-version installs.** Existing target-project installs need to apply the rename locally to align with the new kit convention. On macOS APFS or any case-insensitive filesystem, the naïve `git mv Design design` fails with `Invalid argument`; use the two-step pattern via an intermediate name:
  ...
  Per [ADR-002](design/adr/adr-002-new-project-only-scope.md) the kit explicitly scopes to new projects, so this snippet is a courtesy for users on prior versions, not a contract — pinning to v3.x is supported indefinitely.

### Decisions and policy

- **Mechanical path-string rewrite as an exception to ADR immutability** ([ADR-044](design/adr/adr-044-mechanical-rewrite-immutability-exception.md), [#79](https://github.com/olivermorgan2/workflow-generator/issues/79), shipped 2026-05-07 in PR #81). The kit's standing rule "never edit accepted ADRs in place — supersede instead" gains a narrow exception for purely mechanical path-string rewrites. A rewrite qualifies when six criteria hold: deterministic substitution, no sentence meaning changes, no altered decisions or rationale, no added/removed requirements, uniform application, scriptable and reproducible. Editorial changes still require supersession. PRs invoking this ADR must enumerate (a) the ADR invocation, (b) the exact transformation rule, (c) the runnable command, and (d) the affected ADR numbers in the description.

### Process

- Two issues (#79, #80) sequenced — #79 (accept ADR-044) merged first to establish the authority; #80 (the rename itself) is this v4.0.0 PR. Two prompts, two feature branches. The two-step rename hazard on macOS APFS was caught during planning (Plan agent) before commit 1.
